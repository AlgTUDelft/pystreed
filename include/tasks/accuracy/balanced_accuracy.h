...
namespace STreeD {
	...
	public:
		using SolType = int64_t;			// The data type of the solution
		using SolD2Type = int64_t;			// The data type of the solution in the terminal solver
		using TestSolType = int64_t;		// The data type of the solution that is used for evaluation
		...
		static constexpr int64_t worst = INT64_MAX;		// An UB for the worst solution value possible
		static constexpr int64_t best = 0;				// A LB for the best solution value possible
		static constexpr int64_t minimum_difference = 1;// The minimum difference between two solutions
		...
		int64_t GetLeafCosts(const ADataView& data, const BranchContext& context, int label) const;
		
		// Compute the test leaf costs for the data in the context when assigning label
		int64_t GetTestLeafCosts(const ADataView& data, const BranchContext& context, int label) const;

		// Compute the leaf costs for an instance given a assigned label
		inline void GetInstanceLeafD2Costs(const AInstance* instance, int org_label, int label, int64_t& costs, int multiplier) const {
		...
		inline int64_t GetWorstPerLabel(int label) const { return cost_matrix[label]; }
		
		// Compute the train score from the training solution value
		double ComputeTrainScore(int64_t test_value) const;
		
		// Compute the test score on the training data from the test solution value
		double ComputeTrainTestScore(int64_t test_value) const;
		
		// Compute the test score on the test data from the test solution value
		double ComputeTestTestScore(int64_t test_value) const;
	
	private:
		std::vector<int64_t> cost_matrix, test_cost_matrix;
